...
reactions specified in reactions.txt, using pre-computed geometry optimization
and single-point Gaussian .out files for all molecules found in reactions.txt.

This script can be used in training mode, with transition states specified in
reactions.txt, and in inference mode, without transition specified in
reactions.txt.  Training mode requires the appropriate transition state
Gaussian .out files, and extracts the ground-truth free energy barrier in
addition to the input descriptors.

Run from the command line:
python get_descriptors.py [arguments] /path/to/directory/with/reactions.txt
...
from write_sp_inputs import periodic_table


@lru_cache(maxsize=None)
def read_sp_file(spdir, name, suffix, reacting_atom):
    """
    Function which reads a Gaussian .out output file from a single-point
    computation and extracts the raw features needed to compute the DFT
    descriptors for input into the ML model, returning them as a dictionary.

    The overall filename to be read is the name, followed by a hyphen,
    followed by the suffix, followed by the '.out' extension.

    The output of this function is cached, since multiple reactions may contain
    the same molecule, and thus require reading the same single-point output
    files.

    Arguments:

        - spdir: path to directory where the single-point .out file is located

        - name: base name which identifies the molecule whose raw features are
                to be extracted

        - suffix: filename suffix, which identifies whether the single-point
                  output file corresponds to the neutral molecule ('sp0'),
                  the cation ('sp+'), or the anion ('sp-').

        - reacting_atom: integer identifying the number of the heavy atom in
                         the single-point .out file which undergoes the
                         hydrogen atom transfer (HAT) reaction.

    Returns:

        - features: dictionary with all raw features needed to compute the DFT
                    descriptors for input into the ML model, containing the
                    following keys:

            - 'elements': tuple listing the element symbols of all atoms in
                          the molecule

            - 'coords': molecular geometry as a tuple of 3-tuples.  Each atom
                        in the molecule corresponds to one 3-tuple containing
                        (X-coord, Y-coord, Z-coord).  Note that the order of
                        atoms is the same in 'elements' and 'coords'.

            - 'energy': floating point containing the single-point energy of
                        the molecule in Hartrees

            - 'HOMO': floating point containing the HOMO energy of the
                      molecule in Hartrees

            - 'LUMO': floating point containing the LUMO energy of the
                      molecule in Hartrees

            - 'charge': floating point containing the NBO charge on the
                        reacting_atom

            - 'bonds': tuple containing list of bond orders for all bonds
                       to the reacting_atom.  A bond is defined as a
                       bond order > 0.5

            - 'bondsH': tuple containing list of bond orders for all bonds
                        from hydrogen atoms to the reacting_atom.  A bond
                        is defined as a bond order > 0.5

            - 'bondsX': tuple containing list of bond orders for all bonds
                        from non-hydrogen atoms to the reacting_atom.  A
                        bond is defined as a bond order > 0.5
    """
    ...
    with open(spdir_file_out, 'r') as f:
        for line in f:
            ...
            # Energy line: in the event of unstable wavefunctions, the energy
            # kept will be the last in the file, corresponding to the final
            # stable wavefunction
            elif line.startswith(' SCF Done:'):
                words = line.split()
                energy = float(words[4])

            # A stable wavefunction has been found, and everything below in the
            # output file corresponds to this stable wavefunction
            elif line.strip() == 'The wavefunction is stable under the perturbations considered.':
                stable_wavefunction = True

            # Only collect orbital energies, NBO charges, and Wiberg bond
            # orders after a stable wavefunction has been found
            if stable_wavefunction:
                ...
                if line.strip() == 'Orbital energies and kinetic energies (alpha):':
                    ...
                    # Store the last occupied orbital as the HOMO (by
                    # continually replacing the HOMO energy with each new
                    # occupied orbital)
                    ...
                    # Store the first virtual orbital as the LUMO
                        ...
                        # Prevent later virtual orbitals from replacing LUMO
                        start_orbitals = False
    ...
    features = {
        ...
        'bondsH': tuple(bond for bond, element in zip(bonds, elements)
                        if bond > 0.5 and element == 'H'),
        'bondsX': tuple(bond for bond, element in zip(bonds, elements)
                        if bond > 0.5 and element != 'H'),
    }
...
def get_enthalpy_and_free_energy(optdir, spdir, name):
    """
    Function which reads a series of Gaussian .out output files in order to
    compute the enthalpy and free energy of a molecule.  These ouput files
    include a single-point file, a combined geometry optimization and frequency
    file, and optionally two other single-point files to compute solvation
    corrections.  The output of this function is cached, since multiple
    reactions may contain the same molecule, and thus require reading the same
    single-point output files.

    Arguments:

        - optdir: path to the directory where the optimization and frequency
                  .out file is located

        - spdir: path to the directory where the single-point .out files are
                 located

        - name: base name which identifies the molecule whose enthalpy and
                free energy are to be computed.  The files to be read will have
                this name, followed by a hyphen, followed by a suffix
                (specified in the function body), followed by the '.out'
                extension.

    Returns:

        - enthalpy: floating point containing the enthalpy of the molecule in
                    Hartrees

        - free_energy: floating point containing the Gibbs free energy of the
                       molecule in Hartrees
    """
    ...
    # The solvation correction is computed at a different (lower) level of
    # theory, so it is the difference between the solvated single-point energy
    # and the gas-phase single-point energy at this different (lower) level
    # of theory.
...
def bv(elements, coords, atom):
    """
    Function which computes the buried volume around a particular atom in a
    molecule using radii of 3, 4, and 5 Angstroms.  This function is a wrapper
    around the BuriedVolume functionality in the morfeus software package.
    The output of this function is cached, since multiple reactions may contain
    the same molecule, and thus would require the same buried volume
    computations.

    Arguments:

        - elements: tuple listing the element symbols of all atoms in the
                    molecule

        - coords: molecular geometry as a tuple of 3-tuples.  Each atom in the
                  molecule corresponds to one 3-tuple containing
                  (X-coord, Y-coord, Z-coord).  Note that the order of atoms
                  must be the same in elements and coords.

        - atom: an integer containing the number of the atom around which the
                buried volume is to be computed.
    ...
    bv3 = BuriedVolume(elements, coords, atom, include_hs=True,
                       radius=3, radii_scale=1).fraction_buried_volume
    bv4 = BuriedVolume(elements, coords, atom, include_hs=True,
                       radius=4, radii_scale=1).fraction_buried_volume
    bv5 = BuriedVolume(elements, coords, atom, include_hs=True,
                       radius=5, radii_scale=1).fraction_buried_volume
...
def compute_descriptors(optdir, spdir, reaction, training_mode):
    ...
    hydrogen atom transfer (HAT) reaction into the ML model, returning them
    as a dictionary.

    Arguments:

        - optdir: path to directory where all geometry optimization Gaussian
                  .out output files are located

        - spdir: path to directory where all single-point Gaussian .out output
                 files are located

        - reaction: a dictionary describing the hydrogen atom transfer
                    reaction whose free energy barrier is to be estimated by
                    the ML model, containing the following keys:

            - 'subH_file': base filename for substrate starting material with
                           H atom attached

            - 'cat_file': base filename for catalyst starting material without
                          H atom attached

            - 'sub_file': base filename for substrate product without H atom
                          attached

            - 'catH_file': base filename for catalyst product with H atom
                           attached

            - 'subH_atom', 'cat_atom',
              'sub_atom', 'catH_atom': integer atom number of the heavy atom
                                       that is undergoing hydrogen atom
                                       transfer (HAT) in the respective
                                       molecule

            - 'ts_file': (optional, only for model training) base filename for
                         the hydrogen atom transfer (HAT) transition state

        - training_mode: boolean describing whether or not the model is to be
                         used in training mode. If True, also extracts the
                         ground-truth enthalpy and free energy barriers (this
                         requires the appropriate transition state Gaussian
                         .out files to be present).

    Returns:

        - desc: a dictionary containing all DFT descriptors needed for the
                input of one hydrogen atom transfer (HAT) reaction into the ML
                model.  May optionally also contain the true output
                ('DGdd', 'DHdd') in the case of model training. See the
                function body for all the descriptors and how they are
                calculated.
    """

    desc = {}

    # Compute bond dissociation enthalpy (DH) and bond dissociation free
    # energy (DG) descriptors in kcal/mol
    ...
    # Optionally, in model training, compute the true outputs: enthalpy of
    # activation (DHdd) and free energy of activation (DGdd) in kcal/mol
    if training_mode:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Extracts DFT descriptors \
        used in the machine learning model for all reactions described in \
        reactions.txt, using the geometry optimization and single-point \
        Gaussian .out output files generated previously.  Each line in the  \
        .csv output file (default name: descriptors.csv) corresponds to one \
        line in reactions.txt, and contains all the DFT descriptors needed \
        for the machine learning model to predict the free energy barrier of \
        that reaction.")
    ...
    parser.add_argument(
        "-s", "--sp",
        help="relative path to subdirectory where single-point Gaussian .out \
    ...
    parser.add_argument(
        "-c", "--csv",
        help="base filename of .csv file (without extension) in which to \
              store DFT descriptors",
        default='descriptors'
    )

    parser.add_argument(
        "-t", "--train",
        help="run in training mode, also extracting ground-truth free energy \
              barriers",
        action='store_true'
    )
    ...
    if not os.path.isdir(args.dir):
        raise FileNotFoundError('Dataset directory %s does not exist!'
                                % args.dir)
    ...
    with open(dir_reactions_txt, 'r') as f:
        ...
        for line in f:
            ...
            try:
                ...
                print('Could not extract descriptors for the reaction: '
                      + line.strip())
